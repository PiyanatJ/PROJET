...
app.get("/", (req, res) => {
  // Vous pouvez renvoyer le fichier HTML directement
  res.sendFile(__dirname + "/Public/index.html");
});

// Endpoint pour servir le fichier JavaScript
...
mqttClient.on("message", (topic, message) => {
  ...
  if (topic === "alerte/fuite") {
    ...
    // Lorsque vous recevez des données MQTT, émettez-les à tous les clients connectés
...
// Gérer la connexion du client Socket.IO
io.on("connection", (socket) => {
  console.log("Nouveau client connecté");

  const waterUsageListener = (data) => {
    socket.emit("water-usage-data", data); // Envoyer les données au client
  };

  // Ajouter l'écouteur d'événements à l'événement "water-usage-data" de Socket.IO
  socket.on("water-usage-data", waterUsageListener);

  // Gérer la déconnexion du client
  socket.on("disconnect", () => {
    console.log("Client déconnecté");
    // Retirer l'écouteur d'événements lorsque le client se déconnecte
    socket.off("water-usage-data", waterUsageListener);
  });
});
